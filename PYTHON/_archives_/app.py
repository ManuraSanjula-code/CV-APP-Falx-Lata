...
app.config['SECRET_KEY'] = os.environ.get('JWT_SECRET_KEY', 'bnbewefiduidhwqudh21817161@@@@***')
app.config.update(
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            # Optional: Fetch user details from Redis if needed within the route
            # current_user_data = r.hgetall(f"user:{current_user_username}")
        except jwt.ExpiredSignatureError:
            return jsonify({'message': 'Token has expired'}), 401
        except jwt.InvalidTokenError:
            return jsonify({'message': 'Token is invalid'}), 401

        # Pass the username to the decorated function if needed
        return f(current_user_username, *args, **kwargs)
    return decorated
...
def login():
    ...
    user_key = f"user:{username}"
    stored_hash_bytes = redis_client.hget(user_key, 'password_hash')
    ...
    try:
        if bcrypt.checkpw(password_bytes, stored_hash_bytes):
            ...
            if isinstance(token, bytes):
                ...
                token_str = token # PyJWT 2.0+ returns string
            return jsonify({'token': token_str}), 200
        else:
            return jsonify({'message': 'Invalid credentials'}), 401
    except ValueError as ve: # bcrypt can raise ValueError for invalid salt/hash format
        print(f"BCRYPT Error during login for {username}: {ve}")
        return jsonify({'message': 'Internal server error during authentication'}), 500
    except Exception as e: # Catch other potential errors during JWT encoding
...
def upload_files(current_user_username):
    ...
    for file in files:
        try:
            ...
            cv_data['upload_date'] = datetime.datetime.now().isoformat()
    ...
    if queries:
        ...
        for query in queries:
            ...
            for key in search_keys:
                try:
                    # Decode key if it's bytes (common with redis-py)
                    decoded_key = key.decode('utf-8') if isinstance(key, bytes) else key
                    members = redis_client.smembers(decoded_key)
                    for member in members:
                        try:
                            # Members are likely bytes, decode them
                            cv_ids_for_query.add(member.decode('utf-8') if isinstance(member, bytes) else member)
                        except (UnicodeDecodeError, AttributeError):
                            pass # Skip problematic members
                except (UnicodeDecodeError, AttributeError):
                    pass # Skip problematic keys

            query_cv_id_sets.append(cv_ids_for_query)
            print(f"Query '{query}' matched IDs: {cv_ids_for_query}") # Debug print

        # Combine sets based on logic
        if logic == 'or' and query_cv_id_sets:
            # Union: CVs matching ANY query
            final_cv_ids = set.union(*query_cv_id_sets)
        elif query_cv_id_sets:
            # Intersection (default 'and'): CVs matching ALL queries
            final_cv_ids = set.intersection(*query_cv_id_sets)
        else:
            # No queries produced results
            final_cv_ids = set()

        print(f"Final matched IDs ({logic} logic): {final_cv_ids}") # Debug print
        total = len(final_cv_ids)
        start = (page - 1) * per_page
        end = start + per_page
        cv_id_list = list(final_cv_ids)
        paginated_cv_ids = cv_id_list[start:end]

        for cv_id in paginated_cv_ids:
            try:
                # hgetall returns bytes keys/values, format_cv_result handles decoding
                cv_data = redis_client.hgetall(cv_id)
                if cv_data: # This checks if the dictionary is not empty
                    ...
                    total -= 1 # Adjust total if data is missing
            except Exception as e:
                print(f"Error processing CV {cv_id} during search: {e}")
                continue

    else:
        # If no query, return all CVs (paginated)
        ...
        # Sort by key descending (assuming key contains timestamp for recency)
...
@app.route('/api/view/<cv_id>', methods=['PUT'])
@token_required
...
def api_delete_cv(current_user_username, cv_id):
    ...
    try:
        ...
        # We need this to remove the CV ID from specific index sets like index:email:...
        cv_data_bytes = redis_client.hgetall(cv_id)
        cv_data = {}
        if cv_data_bytes:
            # Decode the basic data needed for indexing removal
            for key_bytes, value_bytes in cv_data_bytes.items():
                try:
                    key_str = key_bytes.decode()
                    # Decode simple string values; keep complex ones as bytes for now if needed
                    # For indexing removal, we mostly need strings like email, name
                    try:
                        cv_data[key_str] = json.loads(value_bytes.decode())
                    except (json.JSONDecodeError, UnicodeDecodeError):
                        # If it's not JSON or not decodable, treat as string
                        cv_data[key_str] = value_bytes.decode()
                except UnicodeDecodeError:
                    # Key couldn't be decoded, skip or handle as needed
                    pass

        # Start a Redis Pipeline for atomic operations ---
        pipe = redis_client.pipeline()

        # Remove the main CV hash data ---
        pipe.delete(cv_id)

        # Remove the CV ID from all general indexes it might be in ---
        # Get all index keys
        all_index_keys = redis_client.keys("index:*")
        for key in all_index_keys:
            try:
                # Decode the key if it's bytes
                decoded_key = key.decode('utf-8') if isinstance(key, bytes) else key
                # Remove the cv_id from this set
                pipe.srem(decoded_key, cv_id)
            except (UnicodeDecodeError, Exception) as e:
        ...
        print(f"Redis error deleting CV {cv_id}: {re}")
...
def get_audit_logs():
    ...
    try:
        ...
        raw_logs = redis_client.lrange('audit_log', start, end)
        total_logs = redis_client.llen('audit_log')
        ...
        for log_str in raw_logs:
            try:
                ...
                print(f"Error decoding audit log entry: {e}. Raw: {log_str}")
                parsed_logs.append({'error': 'Malformed log entry', 'raw': log_str})
        ...
        print(f"Redis error fetching audit logs: {re}")
